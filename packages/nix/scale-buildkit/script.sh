...
case $1 in
   ...
   --turn-on)
       ...
       if [[ $CURRENT_REPLICAS -eq 0 ]]; then
        ...
        while true; do
            ...
            >&2 echo "Waiting for at least one buildkit replica to become available..."
            CURRENT_TIME=$(date +%s)
            ELAPSED_TIME=$((CURRENT_TIME - START_TIME))

            if [[ $ELAPSED_TIME -ge $TIMEOUT ]]; then
                >&2 echo  "Timeout reached while waiting for statefulset to scale up."
                exit 1
            fi
   ...
   --attempt-scale-down)
       if [[ ! $2 =~ ^[0-9]+$ ]]; then
           >&2 echo "Please provide a valid numeric argument for --attempt-scale-down"
           exit 1
       fi
       ...
       >&2 echo  "LAST_BUILD: $LAST_BUILD"

       if [[ -z "$LAST_BUILD" || $((CURRENT_TIME - LAST_BUILD)) -gt $THRESHOLD ]]; then
           >&2 echo  "Last build occurred over $THRESHOLD seconds ago. Scaling down..."
           kubectl scale statefulset $STATEFULSET_NAME --namespace=$NAMESPACE --replicas=0
       else
           >&2 echo  "Last build occurred less than $THRESHOLD seconds ago. Skipping scale down."
       fi
       ;;

   *)
       >&2 echo "Usage: $0 [--record-build|--turn-on|--attempt-scale-down <seconds>]"
       exit 1
       ;;
esac
